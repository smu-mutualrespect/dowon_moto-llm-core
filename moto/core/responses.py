...
class BaseResponse(_TemplateEnvironmentMixin, ActionAuthenticatorMixin):
    ...
    def setup_class(
        ...
        self._remote_addr: str = getattr(request, "remote_addr", None) or ""
    ...
    def call_action(self) -> TYPE_RESPONSE:
        ...
        if action in method_names:
            ...
            try:
                ...
                # 핸들러는 찾았지만 내부 구현이 비어 있을 때 turn_agent로 넘긴다.
                from moto.core.llm_agents import turn_agent
                resp_headers, resp_body = turn_agent.run(
                    url=self.uri,
                    headers=dict(self.headers),
        ...
        # action을 못 읽었거나, 핸들러가 없을 때 turn_agent로 넘긴다.
        from moto.core.llm_agents import turn_agent
        resp_headers, resp_body = turn_agent.run(
            url=self.uri,
            headers=dict(self.headers),
    ...
    def _honeypot_body(self) -> Any:
        content_type = self.headers.get("Content-Type", "") or self.headers.get("content-type", "")
        target = self.headers.get("X-Amz-Target") or self.headers.get("x-amz-target")
        if "json" in content_type.lower() or target:
            return self.body
        return self._get_params() or self.body
